...
def main() -> None:
    ...
    Thực hiện cấu hình Logger, áp dụng chốt chặn cuối tuần và ngày lễ,
    sau đó kích hoạt luồng chạy daily tự động.
    """
    logger: logging.Logger = setup_logger(Config.DEFAULT_LOGGER_NAME)

    # Thực hiện validate cấu hình một cách tường minh tại runtime khởi chạy
    ...
    with Notifier(logger) as notifier:
        try:
            ...
            extractor: VnstockExtractorETL = VnstockExtractorETL(notifier=notifier)
            extractor.run()
        except Exception as e:
            logger.error(
                f"🚨 Sự cố nghiêm trọng sập hệ thống toàn cục: {e}", exc_info=True
            )
            try:
                notifier.send_alert(
